...
()=>{
         ...
         atom.tooltips.add(elms.item(0), {title: 'This is a tooltip',trigger: 'click',placement: 'left', class:'.my-tooltip'});
  ...
  wrongReturnType: function wrongReturnType(node,expected,found,statement) {
    ...
    self.error('Type error in \'' + statement.text + '\'.', details);
    self.highlight(node);
  },
  undefinedArgError: function undefinedArgError(node){
    self.error('Undefined function argument used in \'' + node.text + '\'');
    self.highlight(node);
  },
