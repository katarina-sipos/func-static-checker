let self = module.exports = {
    ...
    /**
     * This creates error notification with the Atom NotificationManager.
     * @param message - string containing the main message of the error
     * @param msg_details - details of the message
     * @param node - node that triggered the error so that the line number can be counted
     */
    ...
    /**
     * Marks the buffer range which is the node area in the editor and
     * then sets the marker  CSS class to  the my-error class so that
     * the line can be highlighted accordingly.
    ...
    highlightError: function (node) {
        ...
        atom.workspace.getActiveTextEditor().decorateMarker(self.marker, {
            type: 'line', //specifies the way the area should be highlighted
            class: 'my-error' //CSS class name
        });
    },
    /**
     * Marks the buffer range which is the node area in the editor and
     * then sets the marker  CSS class to  the my-warning class so that
     * the line can be highlighted accordingly.
     * @param node - node object that triggered the warning.
     */
    ...
    camelCaseWarning: function(node) {
        let line = atom.workspace.getActiveTextEditor().bufferPositionForScreenPosition([node.startPosition.row, node.startPosition.row]);
        line = line.row + 1;
    ...
    /**
     * This creates warning notification with the Atom NotificationManager.
     * @param message - string containing the main message of the warning
     * @param details - details of the message
     * @param node - node that triggered the warning so that the line number can be counted
     */
