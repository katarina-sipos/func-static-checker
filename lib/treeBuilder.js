let self = module.exports = {
  ...
  getType: function getType(item) {
    let type = "";
    if (item.content.firstNamedChild.type === 'binary_expression') {
  ...
  populate: function populate(node) {
    ...
    tree = "";
    toVisit = self.sortByLevel(toVisit);//self.sortByParent(toVisit);
  ...
  getNodes: function getNodes(node, parent) {
    self.result = [];
    self.forest = [];
    //let visited=self.bfs(node);//(node.firstNamedChild.firstNamedChild.firstNamedChild);
    //console.log(visited.filter(i=>i.type=='expression').map(i=>i.text));
