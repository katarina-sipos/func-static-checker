...
//.find(i=>i.getTitle()==='Typing rules tree view')

        //atom.workspace.getLeftDock().hide();
        atom.workspace.open(myModel);
      //let panel=atom.workspace.addLeftPanel({item: myModel,priority:500});
      //atom.workspace.addModalPanel({item: myModel,priority:1000});
      //atom.workspace.panelForItem({item: myModel,priority:1000})
    ...
    /*  Object.keys(typecheck.trees).reverse().forEach(function(key) {
      ...
      /*content="<span>";
      treeBuilder.cluster.forEach((typetree, i) => {
        let last=typetree.pop();

        //typecheck.forest=typecheck.forest.pop();
        typetree.forEach((item, i) => {
          content=content+'<span style="display:inline-block">'+item.join('<br>')+'</span>'+'&ensp;';
        });
        content=content+'<br>'+last+'<br><br>';
      });
    ...
    /*  content=typecheck.result.join('<br>');
      content='<span><span style="display:inline-block">'
      +content+'</span>'+'&ensp;'+'<span style="display:inline-block">5+6<br>8+3</span></span>';
      */
