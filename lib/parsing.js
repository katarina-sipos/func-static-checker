var self = module.exports = {
  parsingAction: function parsingAction(data) {
    ...
    if (tree.rootNode.hasError()) {
      ...
      check.duplicitCode();
     /*Object.keys(typecheck.trees).forEach(function(key) {
     console.log(key, typecheck.trees[key]);
});*/
    /*  typecheck.expression.forEach((expr, i) => {
        console.log(expr);
        console.log(typecheck.items.filter(item => item.expression == expr)
        .map(item => item.text));
      });
*/
      /*typecheck.typeEnv.forEach(item => {
        typecheck.typeEnv.forEach(i => {
          if(item.name!== i.name && item.body!='' && i.body !='' && item.body!=null && i.body !=null)
           check.compare(item,i);
        });
