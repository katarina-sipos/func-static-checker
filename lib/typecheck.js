 var self = module.exports = {
  ...
  determineType: function determineType(toResolve,app) {
    ...
    //console.log(toResolve.firstChild.namedChildren.length);
    //console.log(toResolve.firstChild.toString());
    if(toResolve.type=='expression'){
      if(toResolve.firstChild.type=='const'){
        ...
        //TODO: ako rozanalyzujem zlozene vyrazy?
        //console.log(toResolve.firstChild.firstChild.type.toString());
        if( toResolve.firstChild.firstChild.type =='add'  || toResolve.firstChild.firstChild.type =='mul' ){
          //console.log(toResolve.firstChild.firstChild.namedChildren);
          toResolve.firstChild.firstChild.namedChildren.forEach( n =>{
              /*if(n.type == 'binary_expression'){
                console.log('JE to bin + 'n.text);
              }*/

              //else {console.log(n.text +' : '+self.determineType(n));}
              if(n.firstChild.type != 'binary_expression'){
                var ty = self.determineType(n);
                console.log(n.firstChild.type+' : '+ty);
                if( ty.toString() !== 'int'){
                  display.error('Type error in : ' + app.text);//toResolve.firstChild.text);
                }
                //self.kids.push(self.determineType(n));
                //console.log(self.determineType(n));
              }
              self.determineType(n)
            //  console.log(n.firstChild.type);
              //console.log(self.determineType(n));
          });
        }
