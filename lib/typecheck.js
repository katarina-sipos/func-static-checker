  ...
  use: function use(arr, app) {
    ...
    if(app.namedChildren[1]==null){
      ...
      numOfUsedArgs = app.namedChildren[1].namedChildren.length;
        //var originalString =app.namedChildren[1].text;  //found.arg_types.text; //arr[1].text;
      //console.log(app.type);
      //const splitString = originalString.split(",");

//      console.log(found.arg_list.namedChildren.length+ ' '+splitString.length);
      //len = splitString.length;
    }

    console.log(found.arg_types.namedChildren.length);
    ...
    if (toResolve.type == 'expression') {
      if (node.type == 'const') {
        ...
        /*  }
            ...
            console.log("im here im waiting ");
          }*/
        /*  if(app==null ){
            display.error('Undefined function argument used in \''+app.text+'\'');
            return 'undefined type';
      ...
      // TODO: message builder or sth like that
        ...
        ///  toReturn = 'string';
          ...
          //  if (n.firstChild.type != 'binary_expression') {
              var nType = self.determineType(n, app);
              //if(nType == 'undefined type') toReturn = 'undefined type';

              //console.log(n.firstChild.type+' : '+nType);
              if (nType.toString() !== 'string') {
              //  console.log(nType.toString());
                var details = '\'' + childNode.type + '\'';
                details = details + ' cannot be applied to type \''
                details = details + nType.toString() + '\'';
                display.error('Type error in : ' + node.text, details);
        ...
        //console.log('unary');
        //console.log('childnode type: '+childNode.type);
        toReturn = 'bool';
        if (self.isBoolNode(childNode.type)) {
          childNode.namedChildren.forEach(n => {
            //if (n.firstChild.type != 'unary_expression') {
              var nType = self.determineType(n, app);
              //console.log(n.firstChild.type+' : '+nType);
              //  if(nType == 'undefined type') return;
        ...
        } else if (self.isArithmeticNode(childNode.type)) {
          toReturn = 'int';
          childNode.namedChildren.forEach(n => {
            //if (n.firstChild.type != 'unary_expression') {
              var nType = self.determineType(n, app);
