  ...
  perform: function perform(statSeq) {
    ...
    for (let i = 0; i < statCount; i++) {
      ...
      if (node.constructor.name === 'StatementNode') {
        ...
        //  self.loopThrough(statNode[1]);
        //let self.trees=[];
        //console.log(node.firstChild.namedChildren[1].text);
      //  self.trees.push('_____________');
        self.trees.push(node.firstChild.text);
        self.trees.push('_____________');

        self.trees.push(node.firstChild.namedChildren[1].text);
        let arr = node.firstChild.namedChildren[1].namedChildren;
        arr.forEach((item, i) => {
          let a = item.descendantsOfType('function_input', item.startPosition, item.endPosition);
          a.forEach((it, i) => {
            //console.log("item:");
          //  console.log(it.text);
          self.trees.push('_____________');

            self.trees.push(it.text);
            //self.trees.push('_____________');

          });
        });
        /*self.trees.reverse().forEach((item, i) => {
          console.log(item);
        });*/
