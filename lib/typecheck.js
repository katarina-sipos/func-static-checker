...
let self = module.exports = {
  ...
  undefinedArg: function undefinedArg(statNode){
    let leafs = [];
  ...
  define: function define(statNode, def, namefrom) {
    ...
    if(self.undefinedArg(statNode)){
      self.display.undefinedArgError(def);
      //found.body = statNode[1];
    }
    //let numOfDefinedArgs=self.definedArgsCount(statNode);
    //let numOfDeclaredTypes=self.declaredArgsCount(found);
    /*if (found.arg_types.text === 'unit') {
  ...
  useinUse: function useinUse(statNode, app, expr) {
