...
let self = module.exports = {
  ...
  define: function define(statNode, info) {
    ...
    var input = statNode[1];
    var output = statNode[2];
    var numOfDefinedArgs ;
    var numOfDeclaredTypes ;
    console.log(found.ret_type.text);
    ...
    else if(input.type=='arg_list' && output==null){
      //console.log('(x)->()');
      numOfDefinedArgs = statNode[1].namedChildren.length;
    //  console.log(numOfDefinedArgs);
      found.body=null;
      found.arg_list = statNode[1];

    }
    else if(input.type=='body' && output==null){
      //console.log('()->(x)');
      numOfDefinedArgs=0;
      //console.log(numOfDefinedArgs);
    ...
    else if(input.type=='arg_list' && output.type=='body'){
      //console.log('(x)->(x)');
      numOfDefinedArgs = statNode[1].namedChildren.length;
    //  console.log(numOfDefinedArgs);
      found.arg_list = statNode[1];
    ...
    //console.log(found.arg_list);
    //console.log(found.body);

    /*if(statNode[1]==null){
      console.log('()->()');
    }
    else if(statNode[1].type=='arg_list' ){
      console.log('(x)->()');

    }
    else if(statNode[1].type=='body'){
      console.log('()->(x)');
  ...
  use: function use(statNode, app) {
    ...
    for (var i = 0; i < statNode[1].namedChildren.length; i++) {
      ...
      if (found.arg_types.namedChildren[i].text !== type) {
        var details = 'Expected types of agruments: \'' + found.arg_types.text + '\'. ';
