let self = module.exports = {
    ...
    forest: forest=[],
    cluster: cluster=[],
    treeBuilder: treeBuild=require('./treeBuilder'),
    init: function init(){
      self.typeEnv = [];
      self.predefineIte();
      self.trees = [];
    ...
    predefineIte: function predefineIte(){
      let ite= new Function_stat('ite','int',['bool','int','int'],['u','x','x'],'x');
      self.typeEnv.push(ite);

       ite= new Function_stat('ite','bool',['bool','bool','bool'],['u','x','x'],'x');
       self.typeEnv.push(ite);
       ite= new Function_stat('ite','string',['bool','string','string'],['u','x','x'],'x');
       self.typeEnv.push(ite);
    ...
    sortByLevel: function sortByLevel(toSort){
      return toSort.sort((a, b) => (a.lvl !== b.lvl) ? 1 : -1);
        ...
        for (let i = 0; i < statCount; i++) {
            ...
            if (node.constructor.name === 'StatementNode') {
                ...
                if (node.firstChild.type === 'function_declaration') {
                  ...
                  //console.log(statNode[1].firstNamedChild.text);
                  let visited=self.bfs(statNode[1].firstNamedChild);
                  visited=visited.filter(o=>o.content.type=='expression');
                  self.sortByLevel(visited).forEach((item, i) => {
                    console.log(item.content.text+" : "+item.lvl);

                  });

                  //console.log(visited.filter(i=>i.type==='binary_expression').map(o=>o.text));
                  //console.log(visited.filter(i=>i.type==='const').map(o=>o.text));
    ...
    findDepth: function findDepth(node){
   var maxDepth=0;
   ...
   return maxDepth;
    ...
    bfs: function bfs(root) {
      let visited = [];
        let queue = [];
        let current = {content:root,lvl:0};
        let level=0;
      queue.push(current);
      while (queue.length) {
        current = queue.shift();
        ...
        current.content.namedChildren.forEach((item, i) => {
          queue.push({content:item,lvl:current.lvl+1});
        });
      };
      return visited;
    },
        ...
        const found = self.typeEnv.some(fun =>
          (fun.name === statNode[0].text) //&&(fun.arg_types.text===statNode[1].text)
        );
        if (found) {
    ...
    use: function use(statNode, app, caller) {
      if( statNode[0].text==='ite'){
        ...
        let found = self.typeEnv.find(fun => (fun.name === statNode[0].text)
         &&(fun.arg_types.text===statNode[1].text));
         console.log(found);
