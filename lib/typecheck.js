let self = module.exports = {
            ...
            if (node.constructor.name === 'StatementNode') {
                ...
                if (node.firstChild.type === 'function_declaration') {
                  ...
                  if(statNode[1]!=null){
                    ...
                    self.forest.push(self.wrap(node.text) + '<sup style="color:#ad221d">APP</sup>');
    ...
    dfs: function dfs(node){
      ...
      console.log(toVisit.map(i=>i.text).join('\n'));
    },
    bfs: function bfs(node){
      if(node==null) return;
      let str="";
      //console.log(node.namedChildren.map(i=>i.text));
    ...
    if(node.type==='expression'){
      node.firstNamedChild.namedChildren.forEach((item, i) => {
        self.bfs(item);
      });
    }else{
      node.namedChildren.forEach((item, i) => {
    ...
    indent: function indent(previous,content){
      if(previous ==null) return '<span style="text-decoration:overline">'+content+'</span>';
      console.log(previous.text);
      const len=previous.text.length;
      //console.log(previousItem);
      const index=previous.text.indexOf(content);
      const num = len -index;
      content='&nbsp;'.repeat(num)+content;
    ...
    foo: function foo(superstring,item1,item2){
      if(superstring==null){
        return (parent1.text !== parent2.text) || (parent1.type !== parent2.type);
      }
      return superstring.includes(parent1.text) && superstring.includes(parent2.text);
    },
    populate: function populate(node) {
        let toVisit = node.descendantsOfType('expression', node.startPosition, node.endPosition);
        self.str = "";        toVisit = toVisit.sort((a, b) => ((a.parent.text !== b.parent.text) || (a.parent.type !== b.parent.type)) ? 1 : -1);
        toVisit.forEach((item, i) => {
            ...
            self.str = self.str + " " + self.wrap(item.text) + '<sup style="color:#ad221d"> ' + type+'</sup>';
            if (i < toVisit.length - 1){
            // if( !self.foo(self.result.pop(),item.parent, toVisit[i + 1].parent) ){
        ...
        self.result.push(self.str);
      //  self.result.push(self.wrap(node.text) + '<sup style="color:#ad221d">APP</sup>');
      //  self.result.push('<br>');
