...
let self = module.exports = {
            ...
            if (node.constructor.name === 'StatementNode') {
                ...
                if (node.firstChild.type === 'function_declaration') {
                  ...
                  let leafs=[];
                    ...
                    else self.use(statNode, node.firstChild);
                }
            }
        }
        //typeEnv=[];
    ...
    define: function define(statNode, def,namefrom) {
      ...
      leafs.filter( l => l.type=='arg').forEach((item, i) => {

        if( args.some(j => j.text==item.text)==false   ){
          //console.log('nod defined arg');
          control=false;
          return;
        }
      });
