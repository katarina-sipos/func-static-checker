let self = module.exports = {
    ...
    getIteReturnType: function getIteReturnType(node){

    },
    argInIte: function argInIte(node,app,caller){
      if (caller != null) return;
      node.namedChildren.forEach((item, i) => {
        if(self.treeBuilder.bfs(item).some(
          t=>
            t.content.type==='arg'
    ...
    handleIfThenElse: function handleIfThenElse(node,app,caller){
      if(node==null){
      ...
      let inputs = node.namedChildren.map(o=>self.getType(o,app,caller));
      if(inputs[0]!=='bool'){
      ...
      self.check.alwaysTrueExpression(node.firstNamedChild);
    ...
    use: function use(statNode, app, caller) {
      if( statNode[0].text==='ite'){
        ...
        self.check.recursion(statNode[1],app);

        //self.check.recursion(statNode[1].namedChildren,statNode[0]);
