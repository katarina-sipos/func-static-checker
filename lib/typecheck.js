...
let self = module.exports = {
  ...
  undefinedArg: function undefinedArg(statNode, def) {
    let leafs = [];
  ...
  define: function define(statNode, def, namefrom) {
    ...
    self.undefinedArg(statNode, def);
    ...
    found.body.namedChildren.forEach(childNode => {
      if (self.determineType(childNode, def, namefrom) !== found.ret_type.firstChild.text) {
        self.display.wrongReturnType(statNode[0], found.ret_type.firstChild, type, def);
      }
    });
  },
