let self = module.exports = {
    ...
    define: function (statNode, def, caller) {
        ...
        if(self.duplicitArgCheck(found, statNode)) return;
        if (self.check.emptyBody(statNode, found, def)) return;
        self.check.isRecursiveCheck(found.body, caller);
        self.functionBodyTypeCheck(found,def,caller,statNode);
    },
    functionBodyTypeCheck: function(found,def,caller,statNode){
        let type;
    ...
    use: function (statNode, app, caller) {
    ...
    getType: function (toResolve, expr, caller) {
