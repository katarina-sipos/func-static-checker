let self = module.exports = {
  ...
  condtionExpressionCheck: function alwaysTrueExpression(expr){
  ...
  recursion: function recursion(node,caller){
    node.namedChildren.forEach((item, i) => {
      if(item.firstNamedChild.type==='function_application'){
        console.log(item.firstNamedChild.namedChildren[0].text);
        console.log("+" +caller.namedChildren[0].text);
        if(item.firstNamedChild.namedChildren[0].text===caller.namedChildren[0].text){
          self.display.recursiveCall(caller,caller.namedChildren[0].text);
        }
      }
    });
