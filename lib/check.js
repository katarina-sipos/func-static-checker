let self = module.exports = {
  ...
  recursion: function recursion(namedChildren,caller){
    namedChildren.forEach((item, i) => {
      if(item.firstNamedChild.type==='function_application'){
        if(item.firstNamedChild.namedChildren[0].text===caller.text){
          self.display.recursiveCall(caller);
        }
      }
    });
